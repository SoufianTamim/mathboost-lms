...
// Prepare questions JSON for JS (sanitized)
$js_questions = wp_json_encode( $questions, JSON_
HEX_TAG | JSON_HEX_AMP );
?>
...
<div class="qcm-wrap" id="mb-qcm-<?php echo esc_attr( $qcm_id ); ?>">
  ...
  <?php if ( $intro ) : ?>
  <div class="qcm-intro">
    <h2>🎓 <?php esc_html_e( 'Bienvenue dans ce QCM', MB_TEXT_DOMAIN ); ?></h2>
    <div class="qcm-intro-content">
      <?php echo wp_kses_post( wpautop( $intro ) ); ?>
    </div>
    <div class="qcm-intro-rules">
      <strong>📋 <?php esc_html_e( 'Comment bien utiliser ce QCM ?', MB_TEXT_DOMAIN ); ?></strong>
      <ul>
        <li>
        ...
        <li>
          ...
          <span class="rule-desc"><?php esc_html_e( 'Mais surtout de repartir en ayant tout compris !', MB_TEXT_DOMAIN ); ?></span>
        </li>
      </ul>
    </div>
  </div>
  ...
  <div class="qcm-score-bar">
    ...
    <span class="qcm-counter-badge" id="score-counter">0 / <?php echo esc_html( $total ); ?></span>
    <span class="qcm-score-badge" id="score-badge">0 / <?php echo esc_html( $total ); ?></span>
  </div>

  <!-- QUESTIONS rendered by JS -->
  <div id="qcm-questions" data-total="<?php echo esc_attr( $total ); ?>"></div>
...
(function() {
  var MB_QCM_DATA = {
    ...
    questions: <?php echo $js_questions; // already escaped with JSON_HEX_TAG ?>,
    nonce    : '<?php echo esc_js( wp_create_nonce( 'mb_report_nonce' ) ); ?>',
    ajaxUrl  : '<?php echo esc_js( admin_url( 'admin-ajax.php' ) ); ?>'
  };
  window.MB_QCM_DATA = MB_QCM_DATA;
})();
</script>
